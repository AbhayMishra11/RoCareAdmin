...
const Services = () => {
    const [rows, setRows] = useState([
        { id: 1, service: "Web Development", subtitle: "Full Stack Development", price: "$3000", time: "30 days", status: "Published" },
        { id: 2, service: "SEO Optimization", subtitle: "On-page & Off-page SEO", price: "$1500", time: "15 days", status: "UnPublished" },
        { id: 3, service: "Graphic Design", subtitle: "Logo & Branding", price: "$1000", time: "10 days", status: "Published" },
        { id: 4, service: "Digital Marketing", subtitle: "Social Media Marketing", price: "$2000", time: "20 days", status: "UnPublished" },
    ]);
    ...
    const handleClick = () => navigate('/addservice');
    ...
    return (
        <div className="bg-gray-100 p-5">
            <div className="bg-white flex justify-between my-5 p-8 text-gray-600 font-semibold">
                ...
                <div>
                    <label>Search:</label>
                    <input type="text" placeholder="Search service" value={search} onChange={(e) => setSearch(e.target.value)} className="p-2 border rounded ml-2" />
                </div>
                <button onClick={handleClick} className="bg-[#7EC1B1] w-[200px] text-white p-2 rounded-lg">Add Service</button>
            </div>
                ...
                <table className="table-auto w-full border border-gray-400">
                    <thead>
                        <tr className="bg-gray-300 text-xl text-center">
                            <th className="h-[80px]">Sr. No. </th>
                            <th className="p-2"><img src={arrowup} alt="up" className="inline w-4 h-6" /><img src={arrowdown} alt="down" className="inline w-4 h-6 mr-2" />Service</th>
                            <th className="p-2"> <img src={arrowup} alt="up" className="inline w-4 h-6" /><img src={arrowdown} alt="down" className="inline w-4 h-6 mr-2" />Service Subtitle</th>
                    ...
                    <tbody className="text-center">
                        {paginatedRows.map((row, index) => (
                            <tr key={row.id} className={`${index % 2 === 0 ? "bg-gray-100" : "bg-white"} text-gray-500 font-semibold h-[80px]`}>
                                ...
                                <td className="p-2">{row.subtitle}</td>
